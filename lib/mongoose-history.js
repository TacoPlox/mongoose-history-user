...
function historyPlugin(schema, options) {
        ...
        if (diffOnly && !doc.isNew) {
            ...
            for (let key in newDoc) {
                ...
                if (customDiff) {
                    ...
                    //HACK: To handle array emptied (when the diff would return []), we receive a String with '[]'
                    if (customDiff === '[]') {
                        customDiff = [];
                    }
                    diff[key] = customDiff
    ...
    function defaultDiffAlgo(key, newValue, originalValue) {
        ...
        if (originalValue && !newValue) {
            ...
            return 'null';
        } else if (originalValue && originalValue.length && originalValue.length > 0 && newValue.length === 0) {
            //Array was 'deleted' (emptied)
            //HACK: Returns a dummy value, to be transformed from String into an empty array
            return '[]';
        ...
        } else if (newValue && typeof newValue === 'object' && typeof originalValue === 'object') {
            if (newValue instanceof mongoose.Types.ObjectId || originalValue instanceof mongoose.Types.ObjectId) {
                return defaultDiffAlgo(key, newValue.toString(), originalValue.toString());
            }
